...
1. Получите ZIP нужной версии, например `xivgate_agr_collision-1.3.0.zip`.
...
`Optimization Passes` | Сколько вариантов разбиения перебрать, прежде чем оставить лучший; стандартно `1`
...
оставляет вариант с меньшим их числом. Каждый вариант остаётся точным, поэтому
лишние проходы меняют только время генерации, но не форму коллизии.
...
5. **Сборка обратно.** Соседние выпуклые куски объединяются, если добавленный
...
6. **Зазор.** Опорные плоскости оболочки сдвигаются внутрь на половину `Gap`
   ...
   меньшего куска.
7. **Проверки.** Сверяются объём разбиения против исходника, отсутствие граней
